...
MESSAGE = {"user_id": "u1", "experiment_id": "experiment_1"}
...
async def test_clip_lands_on_experiment_and_is_idempotent():
    with tempfile.TemporaryDirectory() as folder:
        ctx = context(folder)
        await repo.put_experiment(ctx.store, experiment())
        calls: list[str] = []
        path = await director.direct(ctx, MESSAGE, generators(calls))
        assert path == "users/u1/experiments/experiment_1/reference.mp4"
        assert await ctx.blobs.exists(path)
        stored = await repo.get_experiment(ctx.store, "experiment_1")
...
async def test_closed_experiment_gets_no_clip():
    with tempfile.TemporaryDirectory() as folder:
        ctx = context(folder)
