"""Pre-flight: the quest's criteria checked on a preview, before upload.
...
home. It checks only the quest's SEEN criteria; the panel and the Judge do
...
def preflight_agent() -> LlmAgent:
    return LlmAgent(
        ...
        name="preflight",
        description="Checks a preview against the quest criteria before upload.",
...
def prompt_for(quest: Quest, technique: Technique) -> str:
    criteria = "\n".join(f"- {c}" for c in quest.criteria.text) or "- (none)"
    return (
        f"Quest: {quest.title} (technique {technique.name}: {technique.cue})\n"
        f"Criteria to check on the preview:\n{criteria}"
    )


async def check(quest: Quest, technique: Technique, preview_jpeg: bytes) -> PreflightOut:
    return await run_agent(
        preflight_agent(),
        prompt=prompt_for(quest, technique),
