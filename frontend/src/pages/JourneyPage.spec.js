...
describe('Journey repeatability evidence', () => {
  ...
  it('routes a terminal Experiment story to its own marked Shot source', () => {
    ...
    expect(wrapper.vm.deconstructionSource.type).toBe('experiment')
    expect(story).toContain('Turn one Shot into a story')
    expect(story).toContain('the same image completely clean at the end')
  ...
  it('opens and downloads every page of a finished visual story', () => {
    ...
    expect(story.get('a[download]').attributes('href')).toBe('/api/blobs/users/user-1/story/page-1.jpg')
    expect(story.get('a[download]').attributes('download')).toBe('Shoots-story-1-01.jpg')
    expect(story.get('button[data-download-story-images]').text()).toContain('Download all images')
