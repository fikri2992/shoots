...
- Story screenshots: [`leading line`](results/android-market-leading-line.png), [`located tarp`](results/android-market-tarp-region.png), [`Technique history and action`](results/android-market-technique-context.png), [`model-located warm/cool pair`](results/android-market-warm-cool-context.png), [`depth planes`](results/android-market-layering-planes.png), and [`Phi inspection guide`](results/android-market-phi-grid.png).
- Five-view Android comparison: [`paths, colour relationship, depth, located obstruction, and inspection guide`](results/android-market-five-views.jpg).
- The Android comparison uses controlled UI-fixture history to exercise the Technique-context states. Its 14-Shot and 8-Shot counts are not claims about this synthetic Inspiration item. The real Analyst run withheld warm/cool from the visual story because only one lens supported it.
...
- Market prompt stress test: the production baseline plus four temporary prompt additions produced five real Analyst outputs for geometry accountability, colour alternatives, depth members, and teaching restraint. The baseline remained the best balanced result. Geometry prompting found cleaner corridor candidates but lost second-lens agreement; colour prompting surfaced warm/cool without corroboration; the depth variant missed a required expected Technique; and restraint removed the checkable next capture. No temporary prompt was promoted. Compare the [five outputs](results/market-story-experiments/comparison.jpg) and inspect the [machine-readable summary](results/market-story-experiments/summary.json).
