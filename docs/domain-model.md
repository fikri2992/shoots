...
The product language below was locked on 2026-08-25. `Quest` is now `Experiment` (the `experiment.*` topics included), `Fault` is now `Finding`, and the skill graph is now the Technique Map with three states; all three renames are complete in code, storage and UI. What remains of the migration is the `skills` storage key, `TechniqueState`, and the 1 to 10 score — stored so old records still load, read by nothing, and not an approved name or value for any new API, prompt, or UI copy. The numbered history retains some legacy identifiers where it describes the implementation that existed at the time. Decisions 41 through 48 supersede that language wherever they conflict.
...
**Analysis.** Everything recorded about one Shot: Evidence, Findings, composition read, measured facts, model observations, and critique. The stored schema still carries a 1 to 10 score and its five element scores so old records load, and nothing reads either: not the Technique Map, not the Journey, not the Keeper, not a claim of Change, not the choice of comparison frame, and not the wire — the read API names its fields one by one rather than publishing the entity, so a new field has to be let out deliberately.
...
**Technique Map.** One longitudinal record per photographer and Technique (`TechniqueState`, `domain/technique_map.py`). Three states: `unobserved`, `observed` once the Evidence has seen it, `recurring` after three corroborated sightings. Corroborated Evidence moves the state and model scores move nothing. Time moves nothing either — what recurred keeps having recurred, so there is no decay and no `rusty`; whether a quiet Technique is worth offering again is a selection question the Scout asks with `stale_ids`.



**Experiment.** One bounded thing to try, selected by Scout from the photographer's record. It has a type, reason, frozen Baseline, Criteria, optional reference, results, Verdicts, and a post-Experiment Change — which together are the Experiment Record. Its type is `explore`, `reproduce`, or `compare`; only `explore` is issued today. Status is `open`, `completed`, `skipped`, or `expired`. A result that misses the Criteria does not close an Experiment unless its design says one attempt is sufficient.
...
**Comparability.** Why a Baseline and the present could, or could not, be set beside each other: `comparable`, `too few shots`, `different arithmetic`, `unknown dimension`, or `unrecorded sample`. Only `too few shots` can resolve itself, so only it is checked again; the rest are written once and settled.
...
**Experiment type.** Which of three questions an Experiment asks: `explore` widens a dimension the archive barely uses, `reproduce` tests whether a Keeper-associated pattern can be made deliberately, and `compare` changes one variable and asks the photographer which result they value. Only `explore` is issued today; the type is recorded on every Experiment so a record written now still says which question it answered.
...
| Director | legacy `experiment.issued` | legacy Experiment and Technique | optional generated reference clip | gemini-3.7-flash, veo-3.1-fast; outside the core product |
...
13. **The reference clip is a nicety the experiment never waits on.** The Scout publishes `experiment.issued` and returns; the Director has Gemini write the Veo prompt, Veo renders 6 s vertical with its own ambient sound, and `experiment.reference_clip` is set when it lands, about a minute later. Veo failing dead-letters and the experiment simply has no clip; an experiment closed meanwhile gets no clip. Lyria was tried and cut: a music bed under a reference clip decorated the bonus list, not the photographer.
...
24. **The Coach acts.** The Live session carries three function declarations: `issue_quest(technique_id, reason)`, `remember(missing_gear, notes)` and `skill_map()`. Function calls are answered by `services/coach.py` inside the relay and echoed to the phone as "→" lines; an experiment issued by voice is an ordinary experiment (Scout, Director, timing). The Listener remains as the post-session fallback. Cost: the Scout's research runs inside the tool call, so the photographer waits ~40 s for the answer; a "working on it" spoken filler is the obvious next step.

25. **One screen, one state, one decision.** The app's home is `Now`, and it renders exactly one of connect / seed / reading / experiment / idle — never a dashboard of all of them. The skill map and the audit trail moved to `Journey`, the grid to `Frames`; a phone gets three tabs and no floating shutter, because a global shutter would upload outside an experiment and so skip pre-flight, which is the only moment a reshoot is still free. Layout follows from that: media full-bleed, one hero line, criteria in plain sentences, the action pinned in the thumb zone, and everything that explains rather than instructs behind a disclosure row. Three type weights (hero, body, meta) and one accent — amber, meaning "the agent decided this" — replace the seven-colour chip vocabulary.

26. **The first experiment is not asked for.** A new account connects Drive, hands over three or four photos it already has (multi-select, straight into the same Drive folder — the Drive app is never opened), and the pipeline narrates itself while they are read. When the Cartographer updates the map, `scout.issue_first` fires if the user has never had an experiment: within about ninety seconds of signing in, the agent has read the photographer's work and set them a task. After that only the daily tick and `experiment.closed` issue experiments, so it can never loop.
...
33. **The map tracks reliability, because reliability is what it can measure.** Promotion used to read `Analysis.score`, which is one number for the whole photograph: a frame demonstrating six techniques handed the same number to all six, so every technique in a good frame was credited with whatever the best one earned. Measured over the corpus that put 32 of 37 skills at a best score of 8 or 9 and 16 of them at solid, from 18 shots — and `diagonals` reached solid on a 9 it had no part in. The rubric cannot separate them either: its five elements correlate at r = 0.89 and the weighted mean tracks `impact` alone at 0.986, so no per-element substitute exists. What *is* about one technique is how the panel saw it, so `domain/technique_map.py` counts **corroborated** attempts — two lenses agreeing at 0.75 or better, both conditions, because the panel admits a single lens at 0.75 and one lens with a habit is one opinion however often it repeats. `observed` is one sighting; `recurring` is three attempts with three corroborated (decision 46 renamed both states and removed the graded ones). Rebuilt over the same corpus, what had been "solid" falls from 16 techniques to 6, and the ones that fall are the ones no second lens ever saw. `best_score` and `last_score` remain on the TechniqueState so old records load, and are now read by nothing at all: the Judge's comparison chooses by Keeper and corroboration (decision 21) and the Coach's briefing carries sightings and corroborations instead. The map claims that a technique is *repeatable*, which the arithmetic supports, and stops claiming it was done *well*, which nothing here measures. Sources and the corpus figures are in `docs/research-findings.md`.
