...
The product language below was locked on 2026-08-25. `Fault` is now `Finding`, and the skill graph is now the Technique Map with three states; both renames are complete in code, storage and UI. What remains of the migration is `Experiment`, the `experiment.*` topics and the 1 to 10 score, tracked in [feature list](feature-list.md) and not approved names for new APIs, prompts, or UI copy. The numbered history retains some legacy identifiers where it describes the implementation that existed at the time. Decisions 41 through 48 supersede that language wherever they conflict.
...
4. **Hard evidence first.** The Judge checks EXIF bounds before looking at vision tags. A experiment whose technique has EXIF bounds cannot pass on vision alone when EXIF is present and fails the bounds.
...
6. **One experiment a day, one open at a time.** Anything added to the folder while a experiment is open is an attempt at it (or a shot tagged with the experiment id by the Shoot button). A failed attempt appends a verdict with feedback and the experiment stays open until its TTL; the first passing verdict closes it and the Scout issues the next. Skipping is the human gate and is logged, never deleted.
...
13. **The reference clip is a nicety the experiment never waits on.** The Scout publishes `experiment.issued` and returns; the Director has Gemini write the Veo prompt, Veo renders 6 s vertical with its own ambient sound, and `experiment.reference_clip` is set when it lands, about a minute later. Veo failing dead-letters and the experiment simply has no clip; a experiment closed meanwhile gets no clip. Lyria was tried and cut: a music bed under a reference clip decorated the bonus list, not the photographer.
...
21. **The bar is your own previous best.** The Judge's feedback gets the photographer's highest-scoring earlier shot for the technique (from `TechniqueState.shot_ids`) as a second image with its observations, and must say in one sentence what improved and what was lost. `Verdict.compared_with` records which shot.

22. **Pre-flight before upload.** For a experiment, the Shoot button sends a 640 px preview to a one-call check of the experiment's SEEN criteria (`agents/preflight.py`, ~8 s) and shows "shoot again" with a concrete fix before the frame is sent. Camera settings are never guessed from a preview; that stays with the Judge.
