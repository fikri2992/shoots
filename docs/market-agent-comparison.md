...
| Persistent memory | SkillState stores per-Technique attempts, corroboration, status, scores, and recent Shot ids. User constraints, Quests, Verdicts, and ActivityEvents persist separately. | `backend/app/domain/skills.py`; `backend/app/domain/entities.py` |
