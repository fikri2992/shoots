...
| P0.17 | Authenticated Phone Source | partial | A revocable pairing token authenticates the four-day demo and survives restart. The transitional screen still asks for a server address and code. A fixed service origin plus native Google sign-in remain release work. |
...
1. Exercise the current Reproduce build end to end on the physical phone.
2. Deploy that exact revision and prove Pub/Sub retry plus the durable Run barrier.
3. Record the four-minute phone-to-Run-to-Journey demonstration and export the architecture diagram.
4. Add native Google sign-in, Compare, structured memory, Scene grouping, and Live Companion work only after submission.
