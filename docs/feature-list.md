...
| P0.9 | Vocabulary migration | partial | Existing Experiment, Finding, Technique Map, Change, Shot, Scene, Shoot, Shoot Record, and Inspiration seams agree. Deconstruction remains target-only and corrected Explore is incomplete. The legacy `skills` Firestore collection key and `TechniqueState` class remain named migration targets. |
...
| P1.6 | Structured learner memory | partial | Shoot/Scene records, independent Technique Map evidence axes, projection invalidation, Tendency Profile, Keepers, typed Scout decisions, Experiment records, and the first Mine/Inspiration correction exist. Remaining: generic scoped Photographer signals, grouping correction, complete intervention outcomes, and bounded recall without promoting inferred personality to fact. |
