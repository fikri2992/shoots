...
| P0.13 | Real-agent quality gate | built | `backend/scripts/check_learning_quality.py` runs the real Ingest and Analyst stages against a versioned manifest and can re-project deterministic receipt fixes over the same saved model output. The accepted 11-case still-Shot run covers six ordinary phone Shots and five deliberate controls: 180 automatic passes, zero failures, 12 explicit review questions, no errors, and developer review against the locked hobbyist perspective. It records input digests, model and prompt versions, Analysis, receipt, artifacts, latency, and source-report provenance. Mean latency is still 39.8 seconds. See [agent quality](agent-quality.md). |
| P0.14 | Cloud deployment | needed | A read-only Windows preflight now verifies the clean SHA, public configuration, APIs, state, and secret-version presence without printing values. It confirmed the service account and Firestore but found eight setup blockers: VAPID contact, release SHA-256, FCM API, GCS bucket, and four backend secret versions. Pub/Sub setup now reconciles endpoint and explicit base-URL audience, creates stage-owned retained DLQs, and grants forwarding IAM. Done still requires explicit approval, exact deployment, live retries, revision readback, and health proof. See [release readiness](release-readiness.md) and [Cloud proof](cloud-proof.md). |
...
| P0.16 | Four-minute continuous demo | partial | [Submission proof](submission-proof.md) now owns the exact four-minute sequence, unedited action segment, claim-to-proof ledger, vocabulary refusals, redaction rules, and recording placeholders. The sequence uses the measured 40-second Analyst latency to explain the three barriers instead of hiding a spinner. Completion still requires the approved Cloud and Xiaomi run, real ids, exact revision, signed APK hash, recording, and published link audit. |
| P0.17 | Authenticated Phone Source | partial | Credential Manager obtains a nonce-bound Google ID token; the backend verifies signature, issuer, audience, expiry, nonce, and verified email before issuing a revocable 30-day encrypted device session. Pairing endpoints remain only for old APKs. [Android setup](android-setup.md) now maps every OAuth, Firebase, App Link, HTTPS, and signing input, while `verifyReleaseConfiguration` refuses blank or in-repository release credentials. Configured sign-in and physical acceptance remain. |
...
| P0.25 | Evidence-grounded Shot annotation | partial | Clean, Finding, Try, and Guide layers draw current regions, placement, horizons, tested crops, Moves, and blown-highlight masks. The receipt selects one default layer; Android and web no longer stack every available mark, camera and subject Moves outrank crop salvage, and one observable check accompanies the Move. The first real-agent corpus retired `no_centre_of_interest`, `split_horizon`, and `off_guide_subject` from new Finding detection because their inputs prove coordinates, not mistakes; legacy records remain readable. Remaining human annotation review and broader corpus coverage keep this partial. |
