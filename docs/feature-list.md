...
| P0.9 | Vocabulary migration | partial | Existing Experiment, Finding, Technique Map, Change, Shot, Scene, and Shoot backend seams agree. Shoot Record has a typed deterministic receipt, but the clients and Scout still lack the complete term. Deconstruction and Inspiration remain target terms. The legacy `skills` Firestore collection key and `TechniqueState` class remain named migration targets. |
...
| P0.34 | Typed Scout learning choice | partial | Each settled Shoot now stores one code-gated `explain`, Keeper-backed `reproduce`, or `silence` decision with exact warrant Shot ids, rejected routes, input and projection versions, execution result, attempt state, and a replay-safe ActivityEvent. Reproduce creates one deterministic Experiment without a writer call. `ask` and corrected `explore` stay explicitly rejected until their real memory and Evidence contracts exist. Client delivery and later intervention outcomes remain. |
...
| P1.6 | Structured learner memory | partial | Shoot/Scene records, independent Technique Map evidence axes, projection invalidation, Tendency Profile, Keepers, typed Scout decisions, and Experiment records exist. Remaining: explicit scoped Photographer signals, Mine/Inspiration and grouping correction, complete intervention outcomes, and bounded recall without promoting inferred personality to fact. |
